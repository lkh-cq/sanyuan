...
10. **合成、转译并校验。** 先在内部验证每个子任务的 `F_T`、禁止损失与证据边界，再按 [reader-facing-analysis.md](references/reader-facing-analysis.md) 把关系结构转译为自然语言结果。内部显式区分事实、关系、推断、缺口与冲突；默认交付只呈现读者理解任务所需的内容，不倾倒内部账本。
...
用户要求“用三元三才整理”时，至少在内部结构中明确保持：三才的三个固定标签、天↔地↔人拓扑、原始观察与关系推断的边界，以及“只记录联系、不代替下游判断”的职责。除非用户明确要求查看框架或审计过程，不要把这些内部编码直接写入交付物。不要用新造术语替代这些标签。
...
默认先给任务结果，并把内部节点、边、路径和证据判断编译为简明、连贯、可独立阅读的自然语言。文献分析正文优先使用连续的段落级分析句，展开“作者写了什么—如何承接或推进—证据支持到哪里”；不要把每一种关系机械拆成项目符号。

不要在默认交付中直接展示 `B_T`、`M_T`、`H_T`、`Z_T`、节点编号、关系代码、裸箭头、YAML 账本或未注释简图。图表只有在显著提升理解时才作为补充，并写全名称、关系含义、证据来源和必要图例。

只有用户明确要求查看框架、内部过程、机器表示或审计记录时，才在主要结果之后附独立审计块。审计块不得取代自然语言结论。
