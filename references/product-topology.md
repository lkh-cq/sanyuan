...
| 建议归属仓库 | `lkh-cq/fullstack-academic`（2026-09-01 登记；创建被 GitHub 集成权限拒绝 403，授权后新会话重试仍 403——集成令牌无仓库创建权限，需用户侧提供具备该权限的令牌或手动创建） |
...
1. **创建 `lkh-cq/fullstack-academic` 仓库**：集成令牌无仓库创建权限（403，授权后重试仍失败）。
   需用户侧手动创建仓库，或提供具备仓库创建权限的令牌；创建后把 `0.4.0a2` 源码推入并打 tag，
   再更新本文件状态。
2. **安装端升级**：主仓发布链已闭合（新版本 tag 已发布）。等安装端校验通过后走正式升级流程
   同步安装包；禁止直接用 GitHub main 的最新版本号覆盖当前已加载版本。
